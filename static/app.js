...
function renderTable(proxies){
  ...
  for(const p of proxies){
    ...
    html+='<tr>'+
      ...
      `<td>${esc(p.protocol||'未知')}</td>`+
      `<td><span class="badge ${escAttr(p.grade||'c')}">${gradeLabel[p.grade]||'未知'}</span></td>`+
      `<td>${esc((p.delay||0).toFixed(0))}ms</td>`+
      `<td>${esc(regionDisp)}</td>`+
      `<td>${esc(p.region||'未知')}</td>`+
      `<td>${esc(p.source||'未知')}</td>`+
      `<td>${esc(p.anon||'未知')}</td>`+
      `<td>${esc(String(p.last_check||'未知').slice(0,10))}</td>`+
      '</tr>';
  }
