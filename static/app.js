...
const STATE = {page:1, pages:1, perPage:50, sort:'delay', asc:true, filters:{grade:'',country:'',protocol:'',delay:'',location:'',search:''}};
let statsCache = null;
const MAX_PAGE = 7;
...
function renderStats(){
  ...
  document.getElementById('v-d').textContent=(grades.d||0).toLocaleString();
...
function readFilters(){
  ...
  STATE.filters.location=(document.querySelector('[name=location]')?.value||'').trim().toLowerCase();
...
function renderTable(proxies){
  ...
  for(const p of proxies){
    ...
    html+='<tr>'+
      ...
      `<td><span class="badge ${escAttr(p.grade||'c')}">${v(gradeLabel[p.grade])}</span></td>`+
      `<td>${esc((p.delay||0).toFixed(0))}ms</td>`+
      `<td>${esc(regionDisp)}</td>`+
      `<td>${esc(v(p.region))}</td>`+
