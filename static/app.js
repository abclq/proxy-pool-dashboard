...
178|      regionDisp=v(COUNTRY_NAME[p.region]||p.location||p.region);
179|    }
180|    html+='<tr>'+
181|      `<td>${esc(p.ip)} <button class="copy-btn" data-copy="${escAttr(p.ip+':'+p.port)}" title="复制">📋</button></td>`+
182|      `<td>${esc(String(p.port))}</td>`+
183|      `<td>${esc(v(p.protocol))}</td>`+
184|      `<td><span class="badge ${escAttr(p.grade||'c')}">${v(gradeLabel[p.grade])}</span></td>`+
185|      `<td>${esc((p.delay||0).toFixed(0))}ms</td>`+
186|      `<td>${esc(regionDisp)}</td>`+
187|      `<td>${esc(v(p.region))}</td>`+
188|      `<td>${esc(v(p.source))}</td>`+
189|      `<td>${esc(v(p.anon))}</td>`+
190|      `<td>${esc(String(v(p.last_check)).slice(0,10))}</td>`+
191|      '</tr>';
192|  }
